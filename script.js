...
const personalizeGuestInvitation = () => {
  if (!guestNameEl) return;

  const guest = getGuestFromUrl();
  if (!guest) {
    guestNameEl.textContent = "Queridos invitados";
    if (guestPassesEl) {
      guestPassesEl.textContent = "";
      guestPassesEl.hidden = true;
    }
    return;
  }
  ...
  if (guestPassesEl) {
    const passLabel = guest.passes === 1 ? "pase reservado" : "pases reservados";
    guestPassesEl.textContent = `${guest.passes} ${passLabel}`;
    guestPassesEl.hidden = false;
  }
};

personalizeGuestInvitation();
